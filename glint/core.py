...
def apply_lut_3d(
    rgb: NDArray[np.float64], lut_3d: NDArray[np.float64]
) -> NDArray[np.float64]:
    """
    Apply a 3D LUT to an RGB array using vectorized trilinear interpolation.
    lut_3d: array of shape (size, size, size, 3) where indexing is [r, g, b].
    """
    size = lut_3d.shape[0]
    # Scale RGB to 0-(size-1) indices
    coords = rgb * (size - 1.0)
    ...
    # Sample the 8 corners from the LUT
...
def srgb_to_slog3(rgb: NDArray[np.float64]) -> NDArray[np.float64]:
    """Convert sRGB to Sony S-Log3."""
    # Simplified but accurate S-Log3 curve
    return np.clip(0.0766 * np.log10(rgb * 0.9 + 0.01) + 0.5, 0, 1)


def slog3_to_srgb(rgb: NDArray[np.float64]) -> NDArray[np.float64]:
    """Convert S-Log3 to sRGB."""
    return np.clip((10 ** ((rgb - 0.5) / 0.0766) - 0.01) / 0.9, 0, 1)
